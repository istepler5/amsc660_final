...
% running variance for the magnetization. Then compare to the analytic
% expression for the mean magnetization and plot the computed and analytic
% mean magnetizations with the mean +/- sqrt(Var(m)). 

%% Initialization
N = 30;                 % dimension of the square lattice
S = ones(30,30);        % initialize all spins up, so s_ij = 1 for all i,j
beta_c = .4408;
kmax = 1e4;             % start low at first, then increase to 10^8
...
for k = 1:kmax
    ...
    if mod(k,1000) == 0 
        fprintf('iter = %d, running mean = %d, running variance = %d \n',...
            k,mu,variance);
    end
